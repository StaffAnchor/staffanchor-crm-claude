  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm mb-5">
      ...
      <div className={`grid gap-4 ${bottom.length ? "grid-cols-2" : "grid-cols-1"}`}>
        ...
        {bottom.length > 0 && (
          <div>
            <p className="text-[10.5px] font-semibold uppercase tracking-wide text-red-500 mb-2 flex items-center gap-1">
...
function LeaderRow({ row, rank, positive }: { row: ClientLeaderRow; rank?: number; positive: boolean }) {
  return (
    <Link
      href={`/clients/${row.id}`}
      className="flex items-center justify-between gap-3 rounded-lg border border-slate-100 hover:border-slate-300 hover:bg-slate-50/70 px-3 py-2 transition-colors"
    >
      <div className="flex items-center gap-2.5 min-w-0">
        {rank && (
          <span className="w-5 h-5 shrink-0 rounded-full bg-slate-100 text-slate-500 text-[10px] font-semibold flex items-center justify-center">
            {rank}
          </span>
        )}
      ...
      <span
        className={`text-sm font-semibold shrink-0 ${positive ? "text-emerald-600" : "text-red-500"}`}
      >
